...
void TmTaskCreate (TCB* pTaskInfo)
{
    ...
    ASSERT_MESSAGE(Tm.TaskList[TaskID] == NULL,
        ...
        for ( i = 0; i < MAX_TASKS; i++)
        {
            ...
            if ( pTask != NULL)
            {
                    ...
                    for ( Mif = 0; (Mif <= MAX_MIF) && (MiFrame > 0); Mif++)
                    {
                        ...
                        if ( MiFrame & 1 )
                        {
                                  ...
                                  TaskPriority < PriorityIndex[Mif][InsertAt-1])
                            {
                                ...
                                Tm.DtTasks[Mif][InsertAt] =
                                    Tm.DtTasks[Mif][InsertAt-1];
                            ...
                            Tm.DtTasks[Mif][InsertAt] = Tm.TaskList[i];
                ...
                else
                {
                    ...
                    // priority
                    while ( InsertAt > 0 &&
                    TaskPriority < Tm.RmtTasks[InsertAt-1]->Priority)
                    {
                        Tm.RmtTasks[InsertAt] = Tm.RmtTasks[InsertAt-1];
                        InsertAt--;
                    }
                    Tm.RmtTasks[InsertAt] = pTask;
...
void TmTick (UINT32 LastIntLvl)
{
    ...
    if (Tm.bStarted)
    {
        ...
        else
        {
            ...
            if ( Tm.NextRmt < Tm.nRmtTasks )
            {
                UINT8 rmt = (UINT8)Tm.NextRmt;
...
void TmScheduleRmtTasks (void)
{
    ...
    for ( i = 0; i < Tm.nRmtTasks; i++)
    {
        ...
        if (Tm.RmtTasks[i]->Rmt.NextMif <= Tm.MifCount &&
            Tm.RmtTasks[i]->Rmt.MifRate != EVENT_DRIVEN_RMT)
        {
            // reschedule True RMT tasks
            if (Tm.RmtTasks[i]->Rmt.MifRate > EVENT_DRIVEN_RMT)
            {
                Tm.RmtTasks[i]->Rmt.NextMif += (UINT32)Tm.RmtTasks[i]->Rmt.MifRate;
            }
            // Help self-scheduling RMTs that are disabled keep time
            else if ( !Tm.RmtTasks[i]->Enabled)
            {
            ...
            if (Tm.RmtTasks[i]->Enabled)
            {
                if (Tm.RmtTasks[i]->State == TASK_IDLE)
                {
                    if ( i < Tm.NextRmt)
                    {
                        Tm.NextRmt = (UINT16)i;
                    }
                    Tm.RmtTasks[i]->State = TASK_SCHEDULED;
                }
                else
                {
                    if ( Tm.RmtTasks[i]->Rmt.MifRate > EVENT_DRIVEN_RMT)
                    {
                        ...
                        ++Tm.RmtTasks[i]->Rmt.OverrunCount;
                        // count all RMT over runs
                        ++Tm.nRmtOverRuns;

                        if (Tm.RmtTasks[i]->Rmt.OverrunCount == 1)
                        {
...
void TmDispatchRmtTasks (void)
{
    ...
    {
        CurTaskIndex = (UINT8)Tm.NextRmt;
        pTask = Tm.RmtTasks[CurTaskIndex];
...
{
  if ( (Tm.TaskList[TaskID] != NULL) && (!Tm.TaskList[TaskID]->Locked))
  {
      Tm.TaskList[TaskID]->Enabled = State;
  }
}
...
void TmScheduleTask (TASK_INDEX TaskID)
{
  if (Tm.TaskList[TaskID] != NULL)
  {
    ...
    {
        Tm.TaskList[TaskID]->State = TASK_SCHEDULED;
        Tm.NextRmt = 0;
    }
  }
}
...
TASK_INDEX TmGetTaskId(char* name)
{
    ...
    for ( i= 0; i < MAX_TASKS; ++i)
    {
        if (strncmp( name, Tm.TaskList[i]->Name, TASK_NAME_SIZE) == 0)
        {
            id = Tm.TaskList[i]->TaskID;
            break;
        }
    }
