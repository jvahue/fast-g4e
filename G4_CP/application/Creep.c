...
static void Creep_ProcessSensor ( CREEP_SENSOR_PTR pSensor, 
                                  CREEP_SENSOR_DATA_PTR pSensorData );
static FLOAT64 Creep_Calculate( CREEP_SENSOR_VAL_PTR pCreepSensorVal, 
                                UINT16 CreepTblId ); 
static void Creep_FindEntries( UINT16 *p1, UINT16 *p0, FLOAT64 *pEntries, 
         ...
         (pCfg->bEnabled == TRUE) )
    {
      ...
      if ( (er_state == ER_STATE_STARTING) || (er_state == ER_STATE_RUNNING) )
      {
            ...
            (pStatus->state == CREEP_STATE_FAULT_IDLE) )
        {
          ...
          pStatus->erTime_ms = 0; 
          /*                        
          // test force bad sensor at 10 sec mark of ER
          m_Test_CntERInterval = 0;             
          // test force bad sensor at 10 sec mark of ER
          */
        }
        /*          
        // test force bad sensor at 10 sec mark of ER
        if ( m_Test_CntERInterval == 10 )
        {
          // Force failure, always after 10 sec 
          m_Creep_Sensors[i].bFailed = TRUE; 
          
          // test force bad sensor at 10 sec mark of ER
          m_Test_CntERInterval++; 
          // test force bad sensor at 10 sec mark of ER
 ...
 *
 * Parameters:  Index - Sensor Object 
 *
...
UINT16 Creep_GetBinaryHdr ( void *pDest, UINT16 nMaxByteSize )
{
  ...
  for (i=0;i<CREEP_MAX_OBJ;i++)
  {
    memcpy ( (void *) pCreepObjHdr->name[0], (void *) pCreepObjCfg->name[0], 
             CREEP_MAX_NAME ); 
                                  ...
                                  CREEP_SENSOR_DATA_PTR pSensorData )
{
  ...
  if (lastTime != pSensorData->lastUpdate) 
  {
    ...
    // Add to current value list.  Note: As of 09/21/12 we don't need slope/offset 
    ...
    else 
    {
      if (pSensorData->bFailed == FALSE) 
      {
        ...
        if (pSensor->sampleCnt != 0) 
        {
          ...
          timeout_ms = pSensorData->rateTime * pSensor->sampleCnt; 
...
FLOAT64 Creep_Calculate ( CREEP_SENSOR_VAL_PTR pCreepSensorVal, UINT16 CreepTblId )
{
  ...
  pSensorData->cnt=0;  // Clear buffer after read/process
  ...
  // pSensorData->cnt=0;  // Clear buffer after read/process.  Buffer if flushed every 
  //                      //   one sec Creep Interval in calling app. 
  ...
  if (!((RowVal < pCreepTbl->RowVal[0]) || (ColVal < pCreepTbl->ColVal[0])))
  {
    ...
    else 
    {
      ...
      {
/*      
        // TBD add ASSERT if &pCreepTbl->RowVal[0] and &pCreepTbl->ColVal[0] are not
        //     long word aligned !!! 
#pragma ghs nowarning 1545  // Ignore alignment in Cfg.  ASSERT() to ensure if not aligned        
        ASSERT ( ((UINT32) &pCreepTbl->RowVal[0] % 4) == 0 ); 
        ASSERT ( ((UINT32) &pCreepTbl->ColVal[0] % 4) == 0 ); 
 ...
 *****************************************************************************/
static 
void Creep_FindEntries( UINT16 *p1, UINT16 *p0, FLOAT64 *pEntries, 
                               FLOAT64 val, UINT16 entriesMax)
{
  UINT16 i; 
 ...
 *
 * Parameters:   type - CREEP_FAULT_TYPE_RESET, CREEP_FAULT_TYPE_SENSOR_LOSS
 *
 * Returns:      none
 *
 * Notes:        none
...
void Creep_CreateFaultLog ( CREEP_FAULT_TYPE type )
{
  ...
  LogWriteETM (APP_ID_CREEP_FAULT, LOG_PRIORITY_LOW, &log, sizeof(CREEP_FAULT_LOG_SUMMARY), 
               NULL); 
               
  // Add to Creep Fault History buffer in EE Memory
  Creep_AddToFaultBuff( APP_ID_CREEP_FAULT ); 
...
#pragma ghs endnowarning // Ignore alignment in Log. 
  ...
  LogWriteETM (APP_ID_CREEP_PBIT, LOG_PRIORITY_LOW, &log, 
               sizeof(CREEP_PBIT_LOG), NULL); 
               
  // Add to Creep Fault History buffer in EE Memory
  Creep_AddToFaultBuff( APP_ID_CREEP_PBIT ); 
} 
...
BOOLEAN Creep_CheckCfgAndData ( void )
{
  ...
  else 
  {
    ...
    // Do not clear creep cfg.  Leave it and on every power up we will get into this path, until 
    //   user takes action to fix. 
    //   For corrupted overall cfg, we should not enter this path, as default reloaded and crc OK. 
  }
  ...
  if ( Creep_RestoreAppData() == TRUE )
  {
    ...
    if (bOkCfgCrc == TRUE) 
    {
      ...
      {
        // We've had change in creep cfg (could also be overall cfg restored to default and prev
        //    creep processing was enable/active with non default crc16 in EE APP DATA). 
  ...
  { 
    ...
    //   default crc16 (in this case we have double fault where EE APP DATA was bad and cfg crc16 was also bad).  
    //   Thus when, real cfg reloaded to fix creep cfg, another log will be recorded above.  Good. 
    // Clear EE APP Data and update its crc16 to cfg crc16 
    memset ( (void *) &m_Creep_AppData, 0, sizeof(m_Creep_AppData) ); 
    // m_Creep_AppData.bInUse = TRUE; 
  ...
  if ( Creep_RestoreAppDataRTC() == TRUE )
  {
    ...
    {
      NV_Write(NV_CREEP_DATA, 0, (void *) &m_Creep_AppData, sizeof(m_Creep_AppData)); 
      NV_Write(NV_CREEP_CNTS_RTC, 0, (void *) &m_Creep_AppDataRTC, sizeof(m_Creep_AppDataRTC) ); 
    }
  } // End if restore _AppDataRTC() was successful 
  // else retrieve of RTC APP DATA failed. 
  //   This could or could not be an issue.  If creep processing was interrupted, then this will 
  //   be a problem.  This is similar to sensor failure.  Do nothing for now.  TBD 
  ...
  for (i=0;i<CREEP_MAX_OBJ;i++)
  {
    ...
    m_Creep_Status[i].interval.nRateCounts = MIFs_PER_SECOND / 
                                             m_Creep_Cfg.creepObj[i].IntervalRate; 
  }
